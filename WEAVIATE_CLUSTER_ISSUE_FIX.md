...
在 Ubuntu 服務器上部署 Dify 時,Weaviate 容器無法正常啟動,持續出現以下錯誤:
...
- 使用 **虛擬機 (HyperKit/QEMU)** 運行 Docker
- 容器有完整的虛擬網路堆疊
- 可以輕鬆獲取到虛擬網路介面的 IP 地址
- Docker 的橋接網路模式能正常提供私有 IP

#### Ubuntu (原生 Docker)
- 直接在 Linux 核心上運行
- 使用 Linux 網路命名空間 (network namespace)
- 在某些配置下,容器內無法檢測到私有 IP 地址
- 特別是當主機網路配置複雜時

### 網路介面檢測機制

```bash
# macOS Docker 容器內通常能看到
ip addr show
# 輸出: eth0 有明確的私有 IP (如 172.17.0.x)

# Ubuntu Docker 容器內可能
ip addr show  
# 輸出: eth0 沒有私有 IP 或 IP 配置不完整
```

### 環境對比表

| 特性 | macOS Docker Desktop | Ubuntu 原生 Docker |
|------|---------------------|-------------------|
| Docker 運行方式 | 虛擬機內 | 原生 Linux 容器 |
| 網路實現 | 完整虛擬化網路 | Linux network namespace |
| 私有 IP 檢測 | ✅ 容易 | ⚠️ 可能失敗 |
| 網路配置複雜度 | 較簡單 | 取決於主機配置 |
| Weaviate 集群初始化 | ✅ 通常成功 | ❌ 可能失敗 |

### 為什麼使用 host 模式能解決?

使用 `network_mode: host` 後:
- **繞過了 Docker 的橋接網路**: 不再依賴 Docker 的網路命名空間
- **容器直接使用主機的網路堆疊**: 可以直接訪問主機的真實網路介面
- **Weaviate 自動獲取 IP**: 能夠檢測到主機的網路介面和 IP 地址
- **避開平台差異**: 不再受 Docker 網路實現差異的影響

這也是為什麼很多生產環境部署文檔會建議使用 host 網路模式來運行需要集群功能的數據庫服務!
