...
async function isUserAdmin(req: NextApiRequest): Promise<boolean> {
  try {
    ...
    if (token.role === 'Administrator') {
      console.log('[chatbot-settings] User is Administrator (from token)');
      return true;
    }
    ...
    console.log('[chatbot-settings] Administrator group check result:', groupResult.rows.length);
    const isAdmin = groupResult.rows.length > 0;
    console.log('[chatbot-settings] Is Administrator:', isAdmin);
