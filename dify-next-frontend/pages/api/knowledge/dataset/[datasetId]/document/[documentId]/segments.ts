...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { datasetId, documentId } = req.query
  ...
  if (!datasetId || !documentId) {
    return res.status(400).json({ message: 'Missing datasetId or documentId' })
  }
  ...
  const url = `${baseUrl}/datasets/${datasetId}/documents/${documentId}/segments`
