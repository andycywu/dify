  ...
  const selectConversation = async (id: string) => {
    try {
      ...
      const formattedMessages = (historyResult.data || []).map((msg: any) => {
        ...
        let content = '';
        // Dify chatflow/messages: user 訊息在 role: 'user'，內容在 query 或 inputs.query/default_input
        // assistant 訊息在 role: 'assistant'，內容在 answer
        if (msg.role === 'user') {
          // 除錯：log user message 結構
          console.log('User message raw:', msg);
          content = msg.query || (msg.inputs && (msg.inputs.query || msg.inputs.default_input)) || msg.content || '';
