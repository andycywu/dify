  ...
  const selectConversation = async (id: string) => {
    try {
      ...
      const formattedMessages = (historyResult.data || []).map((msg: any) => {
        ...
        // Dify created_at 是 Unix epoch 秒數，無論型別都強制轉 number 再 *1000
        const rawCreated = msg.created_at ?? msg.metadata?.created_at;
        const epochSec = Number(rawCreated);
        if (!isNaN(epochSec) && epochSec > 1000000000) {
          ts = new Date(epochSec * 1000);
        } else if (typeof rawCreated === 'string' && !isNaN(Date.parse(rawCreated))) {
          ts = new Date(rawCreated);
        }
