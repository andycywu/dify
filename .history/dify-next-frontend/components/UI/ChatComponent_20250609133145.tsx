...
const ChatComponent: React.FC<ChatComponentProps> = ({
  ...
  console.log('ENABLE_CITATION_AND_SUGGESTED', ENABLE_CITATION_AND_SUGGESTED);
  ...
  const selectConversation = async (id: string) => {
    try {
      ...
      const formattedMessages = (historyResult.data || []).map((msg: any) => {
        ...
        // Dify created_at 是 Unix epoch 秒數
        if (typeof msg.created_at === 'number') {
          ts = new Date(msg.created_at * 1000); // 一律視為秒
        } else if (typeof msg.created_at === 'string' && !isNaN(Number(msg.created_at))) {
          // 若是字串但為數字，也視為 epoch 秒
          ts = new Date(Number(msg.created_at) * 1000);
        } else if (typeof msg.created_at === 'string' && !isNaN(Date.parse(msg.created_at))) {
          // ISO 字串
          ts = new Date(msg.created_at);
        }
        // 若 created_at 無效，嘗試 metadata.created_at
        if (!ts) {
          const metaCreated = msg.metadata?.created_at;
          if (typeof metaCreated === 'number') {
            ts = new Date(metaCreated * 1000);
          } else if (typeof metaCreated === 'string' && !isNaN(Number(metaCreated))) {
            ts = new Date(Number(metaCreated) * 1000);
          } else if (typeof metaCreated === 'string' && !isNaN(Date.parse(metaCreated))) {
            ts = new Date(metaCreated);
          }
        }
        // 若還是無效，fallback: 用現在時間
        if (!ts || isNaN(ts.getTime()) || ts.getFullYear() < 2000) {
          ts = new Date();
        }
  ...
  return (
    <div 
      ...
      style={{
        ...
        height: '600px', // 固定高度，可依需求調整
        maxHeight: '80vh', // 最大高度不超過視窗
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {messages.map(message => {
              ...
              if (isAssistant && ENABLE_CITATION_AND_SUGGESTED && (message as any).metadata) {
                ...
                suggestedQuestions = extractSuggestedQuestions((message as any).metadata);
                console.log('Message', message, 'Citation', citation, 'SuggestedQuestions', suggestedQuestions);
              }
              return (
                <div
                  key={message.id}
                ...
                >
                  ...
                  >
                    ...
                    {/* 建議問題顯示 */}
                    {isAssistant && ENABLE_CITATION_AND_SUGGESTED && suggestedQuestions && (
                      <AssistantSuggestedQuestions questions={suggestedQuestions} onSelect={setInput} />
                    )}
