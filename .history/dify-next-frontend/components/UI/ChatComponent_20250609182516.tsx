...
import AssistantFeedback from './AssistantFeedback'; // 新增：引入 AssistantFeedback 元件
import { parseDifyTimestamp } from '../../utils/parseDifyTimestamp';

interface ChatMessage {
  id: string;
  rawId?: string; // 新增，保留 Dify 原始 messageId
  content: string;
  role: 'user' | 'assistant';
  timestamp: Date;
  metadata?: any; // 新增 metadata
  isLatestAssistant?: boolean; // 新增：標記是否為最後一則 assistant
  fromHistory?: boolean; // 新增：標記是否為歷史訊息
}
...
const ChatComponent: React.FC<ChatComponentProps> = ({
  ...
  const handleSendMessage = async () => {
    ...
    const userMessage: ChatMessage = {
      ...
      role: 'user',
      timestamp: new Date(),
      isLatestAssistant: false, // 新增，確保型別一致
    };
    
    setMessages(prev => {
      // 新增：移除所有 isLatestAssistant 標記
      return prev.map(m => ({ ...m, isLatestAssistant: false as boolean })).concat(userMessage);
    });
    setInput('');
    setLoading(true);
          ...
          >
            <div className="flex items-end">
              ...
              <div className="flex ml-2">
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" style={{ transform: 'rotate(90deg)' }}>
