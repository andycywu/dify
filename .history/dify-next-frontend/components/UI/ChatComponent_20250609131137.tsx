  ...
  const selectConversation = async (id: string) => {
    try {
      ...
      const formattedMessages = (historyResult.data || []).map((msg: any) => {
        let ts: Date;
        // 強化 created_at 處理，避免 1970 問題
        if (typeof msg.created_at === 'number' && msg.created_at > 1000000000000) {
          // 毫秒級 timestamp
          ts = new Date(msg.created_at);
        } else if (typeof msg.created_at === 'number' && msg.created_at > 1000000000) {
          // 秒級 timestamp
          ts = new Date(msg.created_at * 1000);
        } else if (typeof msg.created_at === 'string' && !isNaN(Date.parse(msg.created_at))) {
          // ISO 字串
          ts = new Date(msg.created_at);
        } else {
          // created_at 無效或不存在，fallback: 用 metadata 裡的 created_at
          const metaCreated = msg.metadata?.created_at;
          if (typeof metaCreated === 'number' && metaCreated > 1000000000000)
            ts = new Date(metaCreated);
          else if (typeof metaCreated === 'number' && metaCreated > 1000000000)
            ts = new Date(metaCreated * 1000);
          else if (typeof metaCreated === 'string' && !isNaN(Date.parse(metaCreated)))
            ts = new Date(metaCreated);
          else
            ts = new Date(); // fallback: 現在
        }
        return {
