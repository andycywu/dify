...
const ChatComponent: React.FC<ChatComponentProps> = ({
  ...
  useEffect(() => {
    ...
    // 條件：isHistoryLoading 為 false，且 messages 最後一則 isLatestAssistant 為 true，且 messages 陣列長度大於 1（排除 welcome）
    if (isHistoryLoading) {
      setLatestSuggestedQuestions(undefined);
      return;
    }
    // 僅在用戶主動發送訊息後才 fetch suggestion
    if (messages.length > 1) {
      const lastAssistant = [...messages].reverse().find(m => m.role === 'assistant' && m.isLatestAssistant);
