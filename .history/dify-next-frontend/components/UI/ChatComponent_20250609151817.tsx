...
interface ChatMessage {
  ...
  isLatestAssistant?: boolean; // 新增：標記是否為最後一則 assistant
}
...
const ChatComponent: React.FC<ChatComponentProps> = ({
  ...
  useEffect(() => {
    ...
    // 僅在非歷史紀錄載入時觸發 fetchSuggestedQuestions
    // 判斷條件：messages 最後一則為 assistant 且 isLatestAssistant 為 true，且 messages 陣列不是歷史紀錄載入（可用 conversationId 是否剛變動、或加一個 flag 標記）
    // 這裡用一個簡單判斷：只有在 loading=false 時才自動 fetch（因為 selectConversation 結束時 loading 仍為 false，這裡需加一個 flag）
    // 更嚴謹做法：加一個 isHistoryLoading state
    if (isHistoryLoading) {
      setLatestSuggestedQuestions(undefined);
      return;
    }
  ...
  }, [messages, showSuggestedQuestions, user?.id, isHistoryLoading]);
  ...
  const selectConversation = async (id: string) => {
    ...
    try {
      ...
      // 修正：Dify chatflow/messages API 回傳的歷史訊息，user/assistant 會分開一則一則回傳，需依照順序直接顯示
      const formattedMessages = (historyResult.data || []).flatMap((msg: any, idx: number) => {
        let ts = parseDifyTimestamp(msg.created_at ?? msg.metadata?.created_at);
        const result: ChatMessage[] = [];
        // 若同一筆 msg 同時有 user query 及 assistant answer，需分成兩則訊息
      ...
      for (let i = formattedMessages.length - 1; i >= 0; i--) {
        if (formattedMessages[i].role === 'assistant') {
          lastAssistantIdx = i;
          break;
        }
      }
      if (lastAssistantIdx !== -1) {
        formattedMessages[lastAssistantIdx].isLatestAssistant = true;
      }
  ...
  const handleSendMessage = async () => {
    ...
    setMessages(prev => {
      // 新增：移除所有 isLatestAssistant 標記
      return prev.map(m => ({ ...m, isLatestAssistant: false })).concat(userMessage);
    });
    setInput('');
    setLoading(true);
    ...
    try {
      ...
      const assistantMessage: ChatMessage = {
        ...
        role: 'assistant',
        // 修正：Dify response.created_at 需視為 epoch 秒
        timestamp: typeof response.created_at === 'number' && response.created_at > 1000000000
      ...
      setMessages(prev => {
        // 新增：移除所有 isLatestAssistant 標記，僅新 assistantMessage 為 true
        return prev.map(m => ({ ...m, isLatestAssistant: false })).concat(assistantMessage);
      });
        ...
        >
          <div className="flex-1 overflow-y-auto p-4">
            {messages.map((message, idx) => {
              // console.log(`[RenderMsg ${idx}]`, message); // 除錯用，已關閉
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {isAssistant && showSuggestedQuestions && latestSuggestedQuestions && message.isLatestAssistant && (
