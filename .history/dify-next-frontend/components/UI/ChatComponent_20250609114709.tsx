  ...
  const selectConversation = async (id: string) => {
    try {
      ...
      const formattedMessages = (historyResult.data || []).map((msg: any) => {
        let ts: Date;
        if (msg.created_at && typeof msg.created_at === 'number' && msg.created_at > 1000000000) {
          // 判斷為秒級 timestamp
          ts = new Date(msg.created_at * 1000);
        } else if (msg.created_at && typeof msg.created_at === 'number' && msg.created_at > 1000000000000) {
          // 判斷為毫秒級 timestamp
        ...
        return {
          ...
          timestamp: ts,
          metadata: msg.metadata, // 新增 metadata
        };
      });
