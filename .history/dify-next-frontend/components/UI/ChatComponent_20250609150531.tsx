...
const ChatComponent: React.FC<ChatComponentProps> = ({
  ...
  const selectConversation = async (id: string) => {
    try {
      ...
      const formattedMessages = (historyResult.data || []).map((msg: any, idx: number) => {
        let ts = parseDifyTimestamp(msg.created_at ?? msg.metadata?.created_at);
        let content = '';
        // 除錯：完整 log 每一則訊息
        console.log(`[HistoryMsg ${idx}]`, msg);
        if (msg.role === 'user') {
          // 更嚴謹地處理 user query 來源
          if (typeof msg.query === 'string' && msg.query.trim()) {
            content = msg.query;
          } else if (msg.inputs && typeof msg.inputs.query === 'string' && msg.inputs.query.trim()) {
            content = msg.inputs.query;
          } else if (msg.inputs && typeof msg.inputs.default_input === 'string' && msg.inputs.default_input.trim()) {
            content = msg.inputs.default_input;
          ...
          // 額外除錯：直接 log content
          console.log(`[HistoryMsg ${idx}] user content:`, content);
        ...
        >
          <div className="flex-1 overflow-y-auto p-4">
            {messages.map((message, idx) => {
              // 除錯：渲染階段 log
              console.log(`[RenderMsg ${idx}]`, message);
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {/* 強制顯示內容與 debug */}
                    <div className="whitespace-pre-wrap" style={{color: 'red', fontWeight: 'bold'}}>
