...
const ChatComponent: React.FC<ChatComponentProps> = ({
  ...
  useEffect(() => {
    ...
    // 僅在非歷史紀錄載入且最後一則 assistant 是最新才 fetch suggestion
    if (isHistoryLoading) {
      setLatestSuggestedQuestions(undefined);
      return;
    }
    // 僅在用戶主動發送訊息後才 fetch suggestion，且最後一則 assistant 必須是剛送出的
    if (messages.length > 1) {
      // 僅當最後一則訊息是 assistant 且 isLatestAssistant 為 true，且該訊息 timestamp 在最近 10 秒內才觸發（避免歷史訊息）
      const lastMsg = messages[messages.length - 1];
      const now = Date.now();
      if (
        lastMsg.role === 'assistant' &&
        lastMsg.isLatestAssistant &&
        now - lastMsg.timestamp.getTime() < 10000 // 10 秒內
      ) {
