...
1. **日常開發**：使用 `gitguardian-security-scan.yml`（最穩定）
2. **PR 檢查**：使用 `gitguardian-pr-check.yml`（快速檢查）
3. **完整掃描**：使用 `gitguardian-advanced-scan.yml`（功能最全）
4. **測試環境**：使用 `gitguardian-reliable.yml`（最佳錯誤處理）
