...
# 強制重新構建
echo "🔨 強制重新構建（無快取）..."
cd ../  # 回到根目錄，因為 docker-compose.yml 在 docker/ 但 Dockerfile 在 dify-next-frontend/
DOCKER_BUILDKIT=1 docker-compose -f docker/docker-compose.yml build --no-cache --pull --force-rm dify-next-frontend

# 啟動容器
echo "🚀 啟動前端容器..."
docker-compose -f docker/docker-compose.yml up -d dify-next-frontend
...
if [ -z "$CONTAINER_NAME" ]; then
    echo "❌ 前端容器啟動失敗"
    echo "檢查日誌："
    docker-compose -f docker/docker-compose.yml logs --tail 50 dify-next-frontend
    exit 1
fi
...
echo ""
echo "🚨 如果仍有問題："
echo "   - 檢查容器日誌: docker-compose -f docker/docker-compose.yml logs -f dify-next-frontend"
echo "   - 強制清除瀏覽器所有資料"
echo "   - 確認後端Dify服務正常運行"
